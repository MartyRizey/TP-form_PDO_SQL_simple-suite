...
/*
  ...
  - trim() permet de nettoyer la saisie utilisateur en supprimant les espaces ou d'autres caractères en début et fin de chaîne.
  OU
  -  https://www.php.net/manual/fr/function.filter-input.php -  https://www.php.net/manual/en/filter.constants.php -
...
/*
  - j'appel ma fonction selectPseudo() en lui passant 2 arguments. La connexion à la Bdd ($connectBdd) car je l'ai requis ici, 
    et le speudo à rechercher ($pseudo).
  - Je stocke le résultat de la requête dans $selectPseudoInBdd qui est un objet. Cela me permet de récupérer ou pas le pseudo 
    saisie dans le formulaire, de la Bdd.
...
if(isset($pseudo) && !empty($pseudo)){
  ...
  /*
    - Je vérifie que la la propriété 'pseudo' de mon objet $selectPseudoInBdd n'est pas vide et la compare au peudo saisi par 
      l'utilisateur.
  ...
  */
  if(!empty($selectPseudoInBdd->pseudo) === $pseudo){
    ...
    if(isset($email) && !empty($email) && substr(strstr($email, '@'),0,1)) {
      ...
      /*
        je vérifie que $password existe et qu'il n'est pas vide, qu'il ne fait pas plus de 12 caractères et que le premier 
        caractère n'est pas '<'. Cette option n'est pas forcement nécessaire.
      */
      if(isset($password) && !empty($password) && strlen($password < 12) && substr($password,0,1) != '<'){
        ...
        /*
          - j'appel une fonction d'insertion des données saisies en Bdd en passant en paramètre la connexion à la Bdd ($connectBdd).
          - Et les valeurs saisies dans le formulaire, ainsi que le mot de passe haché $pwdHash.
          - J'ai accès à ma fonction car j'ai requis le fichier ligne 8 dans ce fichier ci.
        */
